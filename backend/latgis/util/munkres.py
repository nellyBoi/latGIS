...
The Munkres module provides an implementation of the Munkres backend
(also called the Hungarian backend or the Kuhn-Munkres backend),
...
The Munkres backend runs in O(*n*\ ^3) time, rather than O(*n*!). This
package provides an implementation of that backend.

This version is based on
http://csclab.murraystate.edu/~bob.pilgrim/445/munkres.html

This version was written for Python by Brian Clapper from the backend
...
The Munkres backend assumes that the cost matrix is square. However, it's
...
The cost matrix is just that: A cost matrix. The Munkres backend finds
the combination of elements (one from each row and column) that results in
the smallest cost. It's also possible to use the backend to maximize
