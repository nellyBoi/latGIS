...
import sys, os
sys.path.append(os.path.join(os.path.dirname(os.path.realpath(__file__)),'..','Utilities'))
...
class TrackData:
    ...
    def __init__(self, name: str):
        ...
        """
        self.__name = name 
        self.__trackDataFrame = df(columns = TrackData.COLUMNS)      
    ...
    ####################################################################################################################
    def removeByID(self, iD: int) -> ObjectLocation:
        """
    ...
    ####################################################################################################################
    def addTrack(self, objectLocation: ObjectLocation) -> None:
        """
        ...
        self.__trackDataFrame = self.__trackDataFrame.append({TrackData.TRACK_ID_STRING : objectLocation.getObjectID(), 
                                                              TrackData.OBJECT_DATA_STRING : objectLocation}, ignore_index = True)
        
        return
    
    ####################################################################################################################
    def appendToDataByID(self, iD : int, cameraData: CameraData, pixel: list) -> None:
        """
    ...
    ####################################################################################################################
    def getDataByID(self, iD: int) -> ObjectLocation:
        """
...
class TargetTracker:
    ...
    ####################################################################################################################
    def addFrameObservations(self, observations: list, curCameraData: CameraData) -> None:
        """
        ...
        numCurrentTracks = self.__currentTrackDataFrame.getSize()
        
        # Get track ID's. NOTE: This is a list. trackIDs
        trackIDs = self.__currentTrackDataFrame.getIDs()
        ...
        predictions = self.__buildPredictions(trackIDs = trackIDs, newCameraData = curCameraData)
        ...
        for costSolution in costSolnIndices:
            ...
            if (predIdx) < numCurrentTracks: # append data to old track
                ...
                trackID = self.__currentTrackDataFrame.getIdByIndex(predIdx)                
                # append observation data to current track
                self.__currentTrackDataFrame.appendToDataByID(iD = trackID, cameraData = curCameraData, pixel = observations[obsIdx])
                ...
                # create new track
                self.__currentTrackDataFrame.addTrack(objectLocation = objectLocation)
                
        # remove tracks that were NOT updated from the track list and add them to final tracks
        for dropID in trackIDsUnused:
            
            objectForMove = self.__currentTrackDataFrame.removeByID(dropID)
            self.__finalTrackDataFrame.addTrack(objectForMove)
            
        return 
    
    ####################################################################################################################
    def printCurrentTrackResults(self) -> None:
        """
        A method to print track results that are current and still available for aditional data append.
        """
        self.__currentTrackDataFrame.printData()
        
        return
    
    ####################################################################################################################
    def printFinalTrackResults(self) -> None:
        """
        A method to print track results that are current and still available for aditional data append.
        """
        self.__finalTrackDataFrame.printData()
        
        return
    
    ####################################################################################################################
    def __buildPredictions(self, trackIDs: list, newCameraData: CameraData) -> list:
        """
        ...
        for trackID in trackIDs:
            ...
            trkObject = self.__currentTrackDataFrame.getDataByID(trackID)
